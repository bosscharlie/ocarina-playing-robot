...
def preprocessing(filename):
    wave, freq = librosa.load(filename, sr=32000)
    now=0
    while(wave[now]==0):
        now=now+1
...
#进行基频提取
def sampling(core_source_wav):
    now=0
    result=[]
    while(now<librosa.get_duration(filename=core_source_wav)):
        wave,freq=librosa.load(core_source_wav,sr=32000,offset=now+60/(bpm*16),duration=60/(bpm*16))  #最小音符时值十六分音符，每个音符采样四次，取第二个采样点
        if(len(wave)==0):
            break
