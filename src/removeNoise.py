...
def start():
    rate = 0
    load_form()
    sound_data, sound_rate = librosa.load("../wav/sound4.wav",sr=16000)
    print("soundrate:",len(sound_data))
    print(sound_data)
    print(sound_rate)
    sound_data.shape = -1
    noise_data, noise_rate = librosa.load("../wav/noise.wav",sr=16000)
    noise_data.shape = -1
...
if __name__ == '__main__':
    load_form()
    sound_data, sound_rate = sf.read("../wav/sound4.wav")
    sound_data.shape = -1
    print(len(sound_data))
    noise_data, noise_rate = sf.read("../wav/sound.wav")
    noise_data.shape = -1
    after = nr.reduce_noise(audio_clip=sound_data, noise_clip=noise_data)
    print(len(after))
    plt.plot(np.arange(len(after)), after, 'r')
